...
class se_gpm2cls_v0_b(nn.Module):
    def __init__(self, resnet, in_channels=[2048], reduce2=[512], gpm_n=[2], se_design='se', n_classes=0):
        ...
        if reduce2[-1] == 2048: self._id += f"-{reduce2[-1]}d"
        self._descirption = "reset50 stage_3(512-d) fuse to stage_5(512-d, 2048-d), 1segpm(2x2)"
        self.backbone = resnet
        self.scale = nn.ModuleList()
        self.se_gpm = nn.ModuleList()
        self.fc2cls = nn.ModuleList()
        ...
        for i in range(len(in_channels)):
            if in_channels[i] == reduce2[i]:
                self.scale.append(nn.Identity())
            else:
                self.scale.append(_conv2d_norm_leaky_relu(in_channels[i], reduce2[i], 1))
            self.se_gpm.append(se_gpm_block(reduce2[i] + 512, gpm_n[i], se_design=se_design))
            self.fc2cls.append(nn.Linear(reduce2[i] + 512, n_classes, bias=False))
...
if __name__ == "__main__":
    ...
    y = net(x)
    print(y.size())
    # torch.save(net.state_dict(), 'abnet.pth')
